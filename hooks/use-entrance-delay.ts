...
import { INTRO_DURATION } from '@/components/motion/cinematic-intro';
 ...
 * O valor vem da própria abertura (INTRO_DURATION), e não de um número
 * repetido aqui: a entrada orquestrada começa junto com a cortina, para o
 * hero já estar em movimento quando o site é revelado. Em navegação interna
 * (ou com reduced-motion, ou com ?intro=0) o atraso é zero.
 */
export function useEntranceDelay(): number {
  ...
  useEffect(() => {
    ...
    setDelay(firstMount && !prefersReduced && !skipped ? INTRO_DURATION : 0);
    setReady(true);
  }, []);
